...
# Script: 01_code/03_models/LPM/LPM_003B.R
#==============================================================================
#
# ALGORITMO: LPM — Especificación B + Feature selection RF + Balanceo
#
# ESPECIFICACIÓN B (con variables compuestas):
#   Elimina los componentes granulares y conserva los índices agregados.
#   Variables excluidas: ciudad, dpto, sin_ocupados, un_solo_ocupado,
#     nivel_educ_max, nivel_educ_jefe, prop_asalariado, prop_cotiza_pension,
#     prop_prima_serv, prop_sub_transp, prop_domestico, prop_fam_sin_rem,
#     prop_jornalero, alguno_* (8 vars), prop_afil_salud, prop_subsidiado,
#     dep_educacion, dep_vivienda, dep_empleo_formal, dep_proteccion,
#     dep_dependencia, estrato_hog, estrato_bajo, estrato_x_zona.
#   El modelo conserva zona, bogota, dummies regionales, doble_ingreso,
#     brecha_educ, indice_formalidad, vulnerabilidad_lab, n_fuentes_no_lab,
#     prop_contributivo, n_privaciones.
#   → Modelo más parsimonioso; coeficientes más interpretables.
#
# QUÉ HACE ESTE SCRIPT:
#
#   FASE 1 – Selección de variables (feature selection):
#     Filtra el ranking de RF_006/feature_matrix.csv por la especificación B
#     (elimina los componentes antes de seleccionar top N).
...
#
# OUTPUTS:
#   02_outputs/models/LPM/LPM_003B/cv_features_results.csv
#   02_outputs/models/LPM/LPM_003B/cv_features_summary.csv
#   02_outputs/models/LPM/LPM_003B/cv_balance_results.csv
#   02_outputs/models/LPM/LPM_003B/cv_balance_summary.csv
#   02_outputs/models/LPM/LPM_003B/features_comparison.png
#   02_outputs/models/LPM/LPM_003B/balance_comparison.png
#   02_outputs/models/LPM/LPM_003B/threshold.png
#   02_outputs/models/LPM/LPM_003B/roc.png
#   02_outputs/models/LPM/LPM_003B/prcurve.png
#   02_outputs/models/LPM/LPM_003B/coefs.png
#   02_outputs/models/LPM/LPM_003B/diagnostics.rds
#   03_submissions/LPM_003B_*.csv
...
K_FOLDS    <- 5L
N_TOPS     <- c(20L, 40L, 60L, NA_integer_)   # NA = usar todas las vars del pool B
TECHNIQUES <- c("baseline", "class_weights", "downsample", "upsample", "smote")

# =============================================================================
# SECCIÓN 2: VARIABLES EXCLUIDAS — ESPECIFICACIÓN B
# =============================================================================
# Se eliminan los componentes granulares y se conservan los índices compuestos.
# El modelo queda con: zona, bogota, costa_caribe, region_pacifico, eje_cafetero,
#   doble_ingreso, brecha_educ, indice_formalidad, vulnerabilidad_lab,
#   n_fuentes_no_lab, prop_contributivo, n_privaciones, y variables continuas
#   que no forman parte de ningún alias.
# Fuente: alias() del LPM.R original. Ver comentarios detallados en LPM.R.

VARS_EXCLUIR_B <- c(
  "id", "pobre",
  # ciudad y dpto: muchos niveles. zona + bogota + dummies regionales cubren
  # la variación geográfica de forma más parsimoniosa
  "ciudad",
  "dpto",
  # Componentes de doble_ingreso = 1 - sin_ocupados - un_solo_ocupado
  "sin_ocupados",
  "un_solo_ocupado",
  # Componentes de brecha_educ = nivel_educ_max - nivel_educ_jefe
  # Nota: nivel_educ_prom permanece en el modelo (no entra en el alias)
  "nivel_educ_max",
  "nivel_educ_jefe",
  ...
  "prop_fam_sin_rem",
  "prop_jornalero",
  # Componentes de n_fuentes_no_lab (8 indicadores alguno_*)
  "alguno_pension_jub",
  "alguno_remesas",
  "alguno_subsidio",
  "alguno_transf_nac",
  ...
  "prop_afil_salud",
  "prop_subsidiado",
  # Componentes de n_privaciones (5 dimensiones IPM)
  "dep_educacion",
  "dep_vivienda",
  "dep_empleo_formal",
  "dep_proteccion",
...
# =============================================================================
# SECCIÓN 4: FEATURE MATRIX Y SUBCONJUNTOS (Especificación B)
# =============================================================================
message("\n== Cargando feature matrix de RF_006 y aplicando especificación B ==")

feature_matrix_rf <- read_csv(feat_mat_path, show_col_types = FALSE) |>
  arrange(rank)

# Preprocesamiento: zona queda como numeric; ciudad/dpto como factor
# (en especificación B se EXCLUYEN ciudad y dpto, pero preparar_X_lpm
# los convierte de todas formas para que la función sea idéntica en ambas specs;
# la exclusión ocurre al filtrar feat_order_B)
...
# Alinear niveles (necesario aunque ciudad/dpto queden excluidos, por si
# algún subconjunto de prueba futuro los incluyera)
...
# Pool de variables para especificación B:
#   1. Presentes en feature_matrix RF_006
#   2. Presentes en ambos datasets
#   3. No en VARS_EXCLUIR_B (se eliminan los componentes, quedan los compuestos)
cols_en_ambos <- intersect(names(X_train_raw), names(X_test_raw))

feat_order_B <- feature_matrix_rf |>
  filter(
    variable %in% cols_en_ambos,
    !variable %in% VARS_EXCLUIR_B
  ) |>
  arrange(rank) |>
  pull(variable)

X_train_pool <- X_train_raw[, feat_order_B, drop = FALSE]
X_test_pool  <- X_test_raw[,  feat_order_B, drop = FALSE]

P_POOL <- length(feat_order_B)
message("Pool de variables disponibles (especificación B): ", P_POOL)
message("NAs en X_train_pool: ", sum(is.na(X_train_pool)))
message("NAs en X_test_pool:  ", sum(is.na(X_test_pool)))

cat("\nTop 10 variables del pool B (por importancia RF_006):\n")
print(head(feat_order_B, 10))
...
crear_subset <- function(n_top) {
  if (is.na(n_top)) {
    ...
    vars  <- feat_order_B[seq_len(n_eff)]
    list(
...
cat("\nSubconjuntos a evaluar (especificación B):\n")
...
if (file.exists(feat_checkpoint)) {
  ...
  message("\n========================================")
  message("FASE 1: Selección de variables — Especificación B")
...
cat("\n-- Resultados Fase 1: F1 por subconjunto (CV-5) — Especificación B --\n")
...
message("  Especificación: B (índices compuestos, sin componentes granulares)")
...
p_feat <- feat_summary |>
  ...
  labs(
    ...
    subtitle = paste0("Esp. B (índices compuestos). Mejor: ", BEST_SUBSET,
                      " (", BEST_N, " vars) | F1 = ", round(BEST_F1_FEAT, 4)),
    x = "Subconjunto", y = "F1 promedio CV-5",
    caption = "Barras ±1 SD. Variables rankeadas por importancia RF_006 tras excluir componentes."
  ) +
...
p_roc <- ggplot(roc_df, aes(x = fpr, y = tpr)) +
  ...
  labs(title    = paste0("LPM: curva ROC — ", MODEL_ID),
       subtitle = sprintf("Esp. B | Subconjunto: %s | Técnica: %s",
...
p_prcurve <- ggplot(pr_curve_df, aes(x = recall, y = precision)) +
  ...
  labs(title    = paste0("LPM: curva Precision-Recall — ", MODEL_ID),
       subtitle = sprintf("AUC-ROC: %.4f | Esp. B | %s | %s",
...
p_coefs <- coefs_df |>
  ...
  labs(title    = paste0("LPM: efectos marginales — ", MODEL_ID),
       subtitle = paste0("Esp. B | Top 20 coeficientes | Subconjunto: ", BEST_SUBSET),
...
# Diagnósticos
saveRDS(
  list(especificacion  = "B",
       vars_excluidas  = VARS_EXCLUIR_B,
       feat_order      = feat_order_B,
       feat_df         = feat_df,
...
sub_name   <- sprintf("LPM_003B_%s_%s_th%03.0f.csv",
...
nueva_fila <- tibble(
  ...
  threshold          = best_th,
  notas              = paste0(
    "LPM Esp.B (indices compuestos, sin componentes). ",
...
cat("\n", strrep("═", 64), "\n")
cat("  ", MODEL_ID, " — RESUMEN FINAL (Especificación B)\n")
cat(strrep("═", 64), "\n")
cat(sprintf("  Especificación     : B — índices compuestos (sin componentes)\n"))
cat(sprintf("  Pool de variables  : %d (de %d en RF_006, excluidos componentes)\n",
