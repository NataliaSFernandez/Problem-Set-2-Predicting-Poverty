...
# Script: 01_code/03_models/XGBoost/XGB_006.R
#==============================================================================
#
# ALGORITMO: XGBoost — Búsqueda aleatoria de HP sin variables construidas
#
# MOTIVACIÓN — POR QUÉ XGB_006 EXISTE:
#   XGB_005 (benchmark actual) usa las 110 variables de train_final.rds, que
#   incluye 16 variables construidas en 01_data_clean_and_merge.R (Bloques 3,
#   8 y 9): transformaciones cuadráticas, logarítmicas e interacciones.
#
#   XGB_006 evalúa la hipótesis de que esas 16 variables son INNECESARIAS para
#   XGBoost, porque el algoritmo:
#     (a) Captura relaciones no lineales internamente mediante splits
#         consecutivos sobre la misma variable (equiv. a cuadráticos/logs).
#     (b) Detecta interacciones automáticamente via splits conjuntos en un
#         árbol (equiv. a términos educ_x_formal, menores_x_desocup, etc.).
#   En este escenario, las variables construidas son redundantes y pueden:
#     - Aumentar el ruido de la búsqueda de HP (más features → más sobreajuste).
#     - Introducir multicolinealidad que distorsiona la importancia de variables.
#     - Inflar artificialmente la complejidad del modelo sin mejorar el F1.
#
#   Hipótesis alternativa (desfavorable): si las 16 variables capturan señales
#   que XGBoost no encuentra eficientemente con el grid de HP actual (por
#   ejemplo, con max_depth bajo), su exclusión puede bajar el F1 OOF.
#   Comparar XGB_006 vs XGB_005 responde directamente esta pregunta.
#
# VARIABLES EXCLUIDAS RESPECTO A XGB_005 (16 en total):
#
...
# ┌─────────────────────────────┬─────────────────────┬─────────────────────┐
# │ Aspecto                     │ XGB_005             │ XGB_006             │
# ├─────────────────────────────┼─────────────────────┼─────────────────────┤
# │ Variables construidas       │ Incluidas (16)      │ EXCLUIDAS (16)      │
# │ N° predictores              │ 110                 │ 110 − 16 = 94       │
# │ Observaciones de Bogotá     │ Incluidas           │ Incluidas           │
...
#   Si XGB_006 supera a XGB_005 → las variables construidas eran ruido.
#   Si XGB_006 queda por debajo  → XGBoost no reconstruye esas señales solo.
#
# INPUTS:
...
#
# OUTPUTS:
#   02_outputs/models/XGBoost/XGB_006/hp_search_results.csv
#   02_outputs/models/XGBoost/XGB_006/cv_balance_summary.csv
#   02_outputs/models/XGBoost/XGB_006/balance_comparison.png
#   02_outputs/models/XGBoost/XGB_006/varimp.png
#   02_outputs/models/XGBoost/XGB_006/threshold.png
#   02_outputs/models/XGBoost/XGB_006/roc.png
#   02_outputs/models/XGBoost/XGB_006/prcurve.png
#   02_outputs/models/XGBoost/XGB_006/diagnostics.rds
#   03_submissions/XGB_006_*.csv
#   02_outputs/model_registry.csv  (append)
#
# TIEMPO ESTIMADO: ~35–55 min con 9 cores (igual que XGB_005)
...
# -----------------------------------------------------------------------------
# VARIABLES CONSTRUIDAS QUE SE EXCLUYEN EN XGB_006
# -----------------------------------------------------------------------------
# XGBoost puede descubrir relaciones no lineales (splits consecutivos) e
# interacciones (splits combinados en un árbol) por sí mismo. Estas 16
# variables fueron construidas explícitamente en 01_data_clean_and_merge.R
# para modelos lineales (LPM, ElasticNet) que no las capturan internamente.
# Para XGBoost representan información redundante que puede añadir ruido.
VARS_EXCLUIR <- c(
...
message("  Hipótesis: variables construidas son redundantes para XGBoost")
message("  Variables excluidas: ", length(VARS_EXCLUIR), " (cuadráticas + logs + interacciones)")
...
message("  Train (original): ", nrow(train), " filas x ", ncol(train), " columnas")
...
# =============================================================================
# Ciudad y dpto como entero ordinal (igual que XGB_005).
# DIFERENCIA vs XGB_005: se eliminan las 16 variables construidas (VARS_EXCLUIR)
# antes de construir la matriz de features.
message("\n== Preprocesando variables ==")
message("  Codificación: ciudad y dpto como entero ordinal (igual que XGB_005)")
message("  Exclusión adicional: ", length(VARS_EXCLUIR), " variables construidas")
...
# Eliminar las 16 variables construidas de ambos conjuntos
# Nota: select(-any_of(...)) ignora silenciosamente los nombres no encontrados,
# por lo que es seguro aunque alguna variable no esté en el dataset.
...
message("  Variables construidas eliminadas de train: ",
        length(vars_encontradas_train), " de ", length(VARS_EXCLUIR), " declaradas")
message("  Variables construidas eliminadas de test : ",
...
message("  Predictores finales: ", P_TOTAL,
        "  (XGB_005 usaba 110 | XGB_006 usa ", P_TOTAL, ")")
...
if (file.exists(hp_checkpoint)) {
  ...
  message("  N_SEARCH=", N_SEARCH, " combos | ", P_TOTAL, " variables (sin las 16 construidas)")
  message("  Grid HP: idéntico a XGB_005 (anti-sobreajuste)")
  message(strrep("=", 65))

  # Grid HP idéntico al de XGB_005 — sin cambios.
  # La razón de usar el mismo grid es mantener comparabilidad directa:
  # cualquier diferencia en F1 OOF se atribuye exclusivamente al cambio
  # en el conjunto de features, no a diferencias en el espacio de HP.
  set.seed(42)
  hp_grid <- tibble(
...
message("  F1_OOF:      ", round(met_final$F1,        4),
        "  (XGB_005 benchmark: ver model_registry)")
...
p_bal <- bal_summary |>
  ...
  labs(
    ...
    subtitle = sprintf(
      "Mejor: %s | F1_OOF = %.4f | th = %.2f  (Sin las 16 variables construidas)",
  ...
  list(
    ...
    cambios_vs_xgb005  = list(
      variables_excluidas = paste0(
        "16 variables construidas: cuadráticas (educ_jefe_sq, educ_prom_sq, ",
      ...
      n_features         = paste0("110 (XGB_005) → ", P_TOTAL, " (XGB_006)"),
      bogota             = "Incluida en entrenamiento (sin cambio vs XGB_005)",
      grid_hp            = "Sin cambio respecto a XGB_005",
      hipotesis          = paste0(
        "XGBoost captura no-linealidades e interacciones internamente; ",
        "las variables construidas son redundantes o añaden ruido."
      )
    ),
...
sub_name   <- sprintf(
  "XGB_006_depth%d_gam%.1f_L2%.0f_eta%.3f_nrounds%d_%s_th%03.0f.csv",
...
nueva_fila <- tibble(
  ...
  threshold          = best_th,
  notas              = paste0(
    "XGBoost sin variables construidas (successor XGB_005). ",
    "Excluidas 16 vars: cuadrát. (educ_jefe_sq, educ_prom_sq, ",
    "ratio_depend_sq, tasa_ocup_sq, hacinamiento_sq, edad_jefe_sq), ",
    "log. (log_num_personas, log_n_menores), ",
    "interact. (educ_x_formal, educ_x_ocup, menores_x_desocup, ",
    "depend_x_informal, rural_x_cta_propia, educ_x_rural, ",
    "subsidiado_x_menores, jefe_mayor_pension). ",
    "Grid HP y técnicas balanceo idénticos a XGB_005. ",
    "Bogotá incluida en train. N_features=", P_TOTAL, ". ",
    "Mejor HP: depth=", best_hp$max_depth,
    " eta=", best_hp$eta,
    " gam=", best_hp$gamma,
    " mcw=", best_hp$min_child_weight,
...
cat(sprintf("  Bogotá          : incluida en train (sin cambio vs XGB_005)\n"))
cat(sprintf("  Submission      : %s\n", sub_path))
cat(strrep("=", 65), "\n\n")
cat("  → Subir a Kaggle y anotar kaggle_public_F1 en model_registry.csv\n")
cat("  → Comparar F1_OOF y Kaggle con XGB_005 para evaluar hipótesis:\n")
cat("     XGB_006 >= XGB_005 → variables construidas eran ruido\n")
cat("     XGB_006 <  XGB_005 → XGBoost no reconstruye esas señales solo\n\n")
